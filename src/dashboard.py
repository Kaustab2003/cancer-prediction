...
# Import new multi-cancer modules
from multi_cancer_pipeline import MultiCancerPredictor
...
with st.sidebar:
    ...
    # App Mode Selection
    app_mode = st.selectbox("Select Mode", ["🏥 Complete Diagnosis", "🏥 Clinical Analysis", "📸 Image Analysis", "🔬 Combined Analysis", "🤖 Dr. AI Assistant", "📊 Patient History", "🌐 Federated Learning"])
...
# Main Content Logic
if app_mode == "🏥 Complete Diagnosis":
    st.markdown("## 🏥 Complete Cancer Diagnosis System")
    st.markdown("""
    <div class='subtitle'>
    Upload patient clinical data and/or medical images for comprehensive cancer screening across multiple cancer types.
    Get detailed staging, prognosis, and personalized prevention recommendations.
    </div>
    """, unsafe_allow_html=True)
    
    # Initialize predictor
    if 'multi_cancer_predictor' not in st.session_state:
        with st.spinner("Loading AI models..."):
            st.session_state.multi_cancer_predictor = MultiCancerPredictor()
            st.session_state.staging_system = CancerStaging()
            st.session_state.prevention_advisor = PreventionAdvisor()
    
    predictor = st.session_state.multi_cancer_predictor
    ...
    with col2:
        st.info(f"🤖 **{models_info['total_models']} AI Models Active:** " + 
                ", ".join([ct.title() for ct in models_info['models'].keys()]))
    
    st.markdown("---")
    
    # Input Section
    st.markdown("### 📋 Patient Information & Data Upload")
    ...
    with col_left:
        ...
        # Cancer type selection
        cancer_type_map = {
            "Auto-detect from data": None,
            "Breast Cancer (Clinical)": "breast",
            "Lung Cancer (Symptoms)": "lung",
            "Skin Cancer (Image)": "skin",
            "Blood Cancer (Microscopy)": "blood"
        }
        
        selected_type = st.selectbox(
            "Cancer Type",
            list(cancer_type_map.keys()),
            help="Select cancer type or let the system auto-detect"
        )
        
        cancer_type_cd = cancer_type_map[selected_type]
        ...
        data_input_method = st.radio(
            "Clinical Data Input Method",
            ["Manual Entry", "Upload CSV", "Upload JSON"]
        )
        ...
        if data_input_method == "Manual Entry":
            ...
            if not cancer_type_cd or cancer_type_cd == "breast":
                st.caption("Breast Cancer Features (Wisconsin Dataset)")
                col_a, col_b = st.columns(2)
                with col_a:
                    clinical_data['radius_mean'] = st.number_input("Radius Mean", value=14.0, format="%.2f")
                    clinical_data['texture_mean'] = st.number_input("Texture Mean", value=19.0, format="%.2f")
                    clinical_data['perimeter_mean'] = st.number_input("Perimeter Mean", value=90.0, format="%.2f")
                    clinical_data['area_mean'] = st.number_input("Area Mean", value=650.0, format="%.2f")
                with col_b:
                    clinical_data['concavity_mean'] = st.number_input("Concavity Mean", value=0.1, format="%.4f")
                    clinical_data['concave points_mean'] = st.number_input("Concave Points Mean", value=0.05, format="%.4f")
                    clinical_data['radius_worst'] = st.number_input("Radius Worst", value=16.0, format="%.2f")
                    clinical_data['area_worst'] = st.number_input("Area Worst", value=880.0, format="%.2f")
            
            elif cancer_type_cd == "lung":
                st.caption("Lung Cancer Symptom Survey")
                col_a, col_b = st.columns(2)
                with col_a:
                    clinical_data['AGE'] = patient_age
                    clinical_data['SMOKING'] = st.selectbox("Smoking Status", [1, 2], format_func=lambda x: "Non-smoker" if x == 1 else "Smoker")
                    clinical_data['YELLOW_FINGERS'] = st.selectbox("Yellow Fingers", [1, 2], format_func=lambda x: "No" if x == 1 else "Yes")
                    clinical_data['ANXIETY'] = st.selectbox("Anxiety", [1, 2], format_func=lambda x: "No" if x == 1 else "Yes")
                    clinical_data['CHRONIC DISEASE'] = st.selectbox("Chronic Disease", [1, 2], format_func=lambda x: "No" if x == 1 else "Yes")
                    clinical_data['FATIGUE'] = st.selectbox("Fatigue", [1, 2], format_func=lambda x: "No" if x == 1 else "Yes")
                with col_b:
                    clinical_data['WHEEZING'] = st.selectbox("Wheezing", [1, 2], format_func=lambda x: "No" if x == 1 else "Yes")
                    clinical_data['COUGHING'] = st.selectbox("Persistent Coughing", [1, 2], format_func=lambda x: "No" if x == 1 else "Yes")
                    clinical_data['SHORTNESS OF BREATH'] = st.selectbox("Shortness of Breath", [1, 2], format_func=lambda x: "No" if x == 1 else "Yes")
                    clinical_data['SWALLOWING DIFFICULTY'] = st.selectbox("Swallowing Difficulty", [1, 2], format_func=lambda x: "No" if x == 1 else "Yes")
                    clinical_data['CHEST PAIN'] = st.selectbox("Chest Pain", [1, 2], format_func=lambda x: "No" if x == 1 else "Yes")
        
        elif data_input_method == "Upload CSV":
            uploaded_csv = st.file_uploader("Upload CSV with clinical features", type=['csv'])
            if uploaded_csv:
                import pandas as pd
            ...
            uploaded_json = st.file_uploader("Upload JSON with clinical features", type=['json'])
            if uploaded_json:
                clinical_data = json.load(uploaded_json)
                st.write("Loaded data:", clinical_data)
    
    with col_right:
        st.markdown("#### 📸 Medical Image Upload")
        
        st.info("💡 Upload microscopy images, dermoscopy images, X-rays, or other medical imaging")
        
        uploaded_image = st.file_uploader(
            "Upload Medical Image",
            type=['png', 'jpg', 'jpeg', 'bmp', 'tiff'],
            help="Supported: Blood cell microscopy, skin lesion photos, radiographs"
        )
        
        medical_image = None
        
        if uploaded_image:
            medical_image = Image.open(uploaded_image)
            st.image(medical_image, caption="Uploaded Medical Image", use_column_width=True)
            
            # Image info
            st.caption(f"Size: {medical_image.size[0]}x{medical_image.size[1]} pixels")
            st.caption(f"Format: {medical_image.format}")
    ...
    with col_opt1:
        include_staging = st.checkbox("Include Staging Analysis", value=True)
    with col_opt2:
        include_recommendations = st.checkbox("Include Prevention Recommendations", value=True)
    with col_opt3:
        generate_pdf_report = st.checkbox("Generate PDF Report", value=False)
    ...
    if st.button("🔬 Run Complete Diagnosis", type="primary", use_container_width=True):
        ...
        # Validate inputs
        has_clinical_data = bool(clinical_data and len(clinical_data) > 0)
        has_image = medical_image is not None
        
        if not has_clinical_data and not has_image:
            st.error("❌ Please provide either clinical data or upload a medical image")
        else:
            # Run prediction
            with st.spinner("🤖 AI models analyzing patient data..."):
                
                # Determine what to predict
                if has_image and has_clinical_data:
                    # Combined analysis - predict both
                    st.info("🔬 Running combined analysis: Clinical + Image data")
                    
                    # Image prediction
                    image_result = predictor.predict(
                        data=medical_image,
                        cancer_type=cancer_type_cd if cancer_type_cd in ['skin', 'blood'] else None,
                        include_staging=include_staging,
                        include_recommendations=include_recommendations
                    )
                    
                    # Clinical prediction
                    clinical_result = predictor.predict(
                        data=clinical_data,
                        cancer_type=cancer_type_cd if cancer_type_cd in ['breast', 'lung'] else None,
                        include_staging=include_staging,
                        include_recommendations=include_recommendations
                    )
                    ...
                    results_to_display = [
                        ("Image-Based Analysis", image_result),
                        ("Clinical Data Analysis", clinical_result)
                    ]
                    ...
                    result = predictor.predict(
                        data=medical_image,
                        cancer_type=cancer_type_cd,
                        include_staging=include_staging,
                        include_recommendations=include_recommendations
                    )
                    st.session_state.diagnosis_result = result
                    results_to_display = [("Image-Based Analysis", result)]
                    ...
                    st.session_state.diagnosis_result = result
                    results_to_display = [("Clinical Data Analysis", result)]
            
            # Display Results
            st.success("✅ Analysis Complete!")
            
            for analysis_name, result in results_to_display:
                st.markdown(f"---")
                ...
                with col_pred3:
                    malignancy_text = "⚠️ MALIGNANT" if result['is_malignant'] else "✅ BENIGN"
                    malignancy_color = "🔴" if result['is_malignant'] else "🟢"
                    st.metric(
                        "Malignancy Status",
                        malignancy_text,
                        delta=malignancy_color
                    )
                ...
                if 'probabilities' in result:
                    ...
                    fig = go.Figure(data=[
                        go.Bar(
                            ...
                            marker_color=['#ff4b4b' if p > 50 else '#4CAF50' for p in prob_df['Probability']],
                ...
                # Staging Information
                if 'staging' in result and result['staging']:
                    st.markdown("#### 🎯 Staging & Risk Assessment")
                    ...
                    with col_stage2:
                        risk_level = staging.get('risk_level', 'Unknown')
                        risk_color = {
                            'Very Low': '🟢',
                            'Low': '🟢',
                            'Moderate': '🟡',
                    ...
                    with col_stage3:
                        if 'prognosis' in staging and '5yr_survival' in staging['prognosis']:
                            survival_rate = staging['prognosis']['5yr_survival']
                            st.metric("5-Year Survival Rate", f"{survival_rate}%")
                    
                    # TNM Classification
                    if 'tnm' in staging:
                        tnm = staging['tnm']
                        st.info(f"📋 TNM Classification: **T{tnm.get('T', '?')} N{tnm.get('N', '?')} M{tnm.get('M', '?')}**")
                    
                    # Additional staging details
                    with st.expander("🔍 Detailed Staging Information"):
                        for key, value in staging.items():
                            if key not in ['stage', 'risk_level', 'tnm', 'prognosis']:
                ...
                if 'recommendations' in result and result['recommendations']:
                    ...
                    with st.expander("🥗 Dietary Recommendations"):
                        diet = rec.get('dietary_recommendations', {})
                        
                        col_diet1, col_diet2 = st.columns(2)
                        
                        with col_diet1:
                            st.markdown("**✅ Foods to Eat:**")
                            for food in diet.get('foods_to_eat', [])[:8]:
                                st.write(food)
                        
                        with col_diet2:
                            st.markdown("**❌ Foods to Avoid:**")
                            for food in diet.get('foods_to_avoid', [])[:8]:
                                st.write(food)
                    
                    # Treatment Options
                    with st.expander("🏥 Treatment Pathways"):
                        treatments = rec.get('treatment_options', {})
                        for treatment_type, options in treatments.items():
                            if options:
                                st.markdown(f"**{treatment_type.replace('_', ' ').title()}:**")
                                for option in options[:5]:  # Show top 5
                                    st.write(f"- {option}")
                    
                    # Screening Schedule
                    with st.expander("📅 Recommended Screening Schedule"):
                        screening = rec.get('screening_schedule', {})
                        if 'follow_up_frequency' in screening:
                            st.info(f"**Follow-up Frequency:** {screening['follow_up_frequency']}")
                        
                        for key, value in screening.items():
                            if key != 'follow_up_frequency' and isinstance(value, list):
                                st.markdown(f"**{key.replace('_', ' ').title()}:**")
                                for item in value[:5]:
                                    st.write(f"- {item}")
                    
                    # Support Resources
                    with st.expander("🆘 Support Resources & Organizations"):
                        resources = rec.get('support_resources', {})
                        for category, orgs in resources.items():
                            st.markdown(f"**{category.replace('_', ' ').title()}:**")
                            for org in orgs[:5]:
                                st.write(f"- {org}")
            ...
            if generate_pdf_report:
                with st.spinner("📄 Generating comprehensive PDF report..."):
                    # Use first result for report
                    report_result = results_to_display[0][1]
                    
                    text_report = predictor.generate_report(
                        report_result,
                        output_format='text',
                        output_file=None
                    )
                    
                    st.text_area("📄 Text Report Preview", text_report, height=400)
                    
                    # Download button
                    st.download_button(
                        label="📥 Download Full Report",
                        data=text_report,
                        file_name=f"cancer_diagnosis_report_{patient_name}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.txt",
                        mime="text/plain"
                    )
